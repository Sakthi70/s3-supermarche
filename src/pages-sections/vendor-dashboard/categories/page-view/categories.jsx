...
const CategoriesPageView = ({}) => {
  ...
  const uploadMultipleCategory =async(arr)=> {
    let bulkArr =  [...new Map(arr.map(item =>
      [item['name'], item])).values()].filter(y =>  !categories
      .filter((x) => x.parentId === slug)
      .some(
        (x) =>
          x.name.trim().toLowerCase() === y.name.trim().toLowerCase()
      ));
      await createMultipleCategory(bulkArr.map(x => {return {...x,parentId:slug== "" ? null : slug,slug: parent ? parent.slug : '' + "/" + x.name.trim()}})).then(async(values) => {
  ...
  const uploadMultipleProducts =async(arr)=> {
    let genProducts = arr.map(x => {
      ...
      return {
        ...
        tags: (!x.tags || x.tags == "") ? [] : x.tags.split(',').map(x => x.trim()),
        images: (!x.images || x.images== "") ? [] :  x.images.split('||').map(x => x.trim()),
        type: (!x.type || x.type == "") ? "Other" : x.type,
        categoryId: slug,
        price:parseFloat(x.price ?? 0),
        salePrice: (!x.salePrice || x.salePrice === "") ? null : parseFloat(x.salePrice),
  ...
  const uploadComps =() => {
    return <>
{slug && <IconButton color="primary" onClick={() =>setopenProduct(true)}>
  <ChromeReaderMode  />
</IconButton>}
  ...
  return (
    <PageWrapper isView={true} type={view} toggleView={(val) => {localStorage.setItem('categoryListOption', val);setview(val);}} title={parent? `${t("Categories Under")} ${parent.name}` : t("Product Categories")}>
      ...
      />
      <BulkUploadCategory open={open} uploadCategory={uploadMultipleCategory}  handleClose={() => setopen(false)}/>
      <BulkUploadProduct open={openProduct} uploadProduct={uploadMultipleProducts}  handleClose={() => setopenProduct(false)}/>
      <Card>
        <Scrollbar>
